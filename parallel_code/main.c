...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  printf("displs[%d] = %d\n",num_procs-1,  displs[num_procs-1]);
  ...
  MPI_Gatherv((&u_bar)->image_data[start], counts[my_rank]-1, MPI_FLOAT, (&whole_image)->image_data[0], counts, displs, MPI_FLOAT, 0, MPI_COMM_WORLD);
  printf("After gather..\n");
  if (my_rank==0) {
